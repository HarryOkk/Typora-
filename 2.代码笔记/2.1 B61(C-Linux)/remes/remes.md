...
typedef struct
{
	...
	uint32_t ele_id;
	uint8_t reserved[12];
	uint8_t run_status_data[65];
} remes_ele_run_status_t;
```
...
函数返回：静态布尔值变量
函数动作：
1.编译时定义一次静态变量：上次操作ID
2.判断当前ID和上一次ID是否重复，并返回
主要功能：判断同一消息类型的REMES消息操作ID是否重复
*/

static uint32_t last_op_id = 0;
static bool remes_op_id_duplicated(uint8_t msg_type, uint32_t op_id)
{
	if(last_op_id != op_id)
	{
		last_op_id = op_id;
